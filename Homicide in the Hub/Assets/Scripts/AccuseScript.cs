...
public class AccuseScript : MonoBehaviour {
	...
	private void AccusationResult(bool accusation){
		...
		if ((accusation == true) && (character.IsMurderer ())) {
			...
			if(NotebookManager.instance.inventory.Size() < 9 ) character.BlockCharacterQuestioning(); //ADDITION BY WEDUNNIT //Only block if there are still clues to find, or the player might get stucku
		}
	}
